 ...
 */
(function($) {
    var limitImage=$('#multi_upload').attr('limitImage');
    var maxSize=$('#multi_upload').attr('maxSize');
    var maxExtensions=$('#multi_upload').attr('maxExtensions');
    var dragText=$('#multi_upload').attr('dragText');
    var btnText=$('#multi_upload').attr('btnText');
    var sizeError=$('#multi_upload').attr('sizeError');
    var errorOnResponse=$('#multi_upload').attr('errorOnResponse');
    var limitError=$('#multi_upload').attr('limitError');
    var delfiletext=$('#multi_upload').attr('delfiletext');
    var invalidExtError=$('#multi_upload').attr('invalidExtError');
    $.fn.pekeUpload = function(options) {
        ...
        var defaults = {
            ...
            allowedExtensions: maxExtensions,
            invalidExtError: invalidExtError,
            maxSize: maxSize,
            sizeError: sizeError,
            showPreview: true,
            showFilename: true,
            ...
            errorOnResponse: errorOnResponse,
            onSubmit: true,
            url: "#",
            data: null,
            limit: limitImage,
            limitError: limitError,
            delfiletext: delfiletext,
            ...
            handlebuttonevents: function() {
                ...
                $(document).on('click','.pkdel',function(){
                    ...
                    if($(this).attr('fileName')){
                        var fileAttrName=$(this).attr('fileName');
                        var oldVal=$('#multi_upload_removed').val();
                        if(oldVal==''){
                            var newVal=fileAttrName;
                        }else{
                            var newVal=oldVal+','+fileAttrName;
                        }
                        $('#multi_upload_removed').val(newVal);
                    }

                    var parent = $(this).parent('div').parent('div');
                    var newLimit=parseInt(options.limit) + 1;
                    var limitation=parseInt($('multi_upload').attr('LimitedUploade'));
                    if(limitation < newLimit){
                        options.limit=limitation;
                    }else{
