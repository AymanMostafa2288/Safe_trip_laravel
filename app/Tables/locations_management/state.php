...
function tableHead($rows)
{
    ...
    $rows['head_select']['db']=['country_id'];
...
function tableBody($rows, $related_to = false)
{
    ...
    $records_count = app(StatesInterface::class)->model->count();
    $page = 1;
    $offset = 0;
    $pagination = false;
    ...
    $body = app(StatesInterface::class)->data($request);
    ...
    $rows["main_url"] = url("dashboard/modules/locations_state/");
...
function tableButtons($rows, $related_to = false)
{
    ...
    $rows["table"]["add"] = true;
    $rows["table"]["add_link"] = route("location.state.create");
    if ($related_to != false) {
        $rows["table"]["add_link"] = route("location.state.create") . "?related_to=" . $related_to;
    }
...
function tableDesign($rows)
{
    ...
    $rows["action"]["name"] = "States Controller";
    return $rows;
}
