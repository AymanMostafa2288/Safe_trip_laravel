...
function table_body($rows)
{
    ...
    $body = app(CounterInterface::class)->data($request);
...
function table_buttons($rows)
{
    ...
    $rows['table']['add'] = true;
    $rows["table"]["add_link"] = route("counters.create");
    $rows["table"]["delete_link"] = url("dashboard/builder/counters/");
    $rows["table"]['edit_link']=url("dashboard/builder/counters/");
...
function table_design($rows)
{
    ...
    $rows["action"]["name"] = "Counters Controller";
    return $rows;
}
