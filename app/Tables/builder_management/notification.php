...
function table_body($rows, $related_to = false)
{
    ...
    $records_count = app(NotificationInterface::class)->model->count();
    $page = 1;
    $offset = 0;
    $pagination = false;
    ...
    $body = app(NotificationInterface::class)->data($request);
    ...
    $rows["main_url"] =  route("notifications.create");
...
function table_buttons($rows, $related_to = false)
{
    ...
    $rows["table"]["add_link"] = route("notifications.create");
    if ($related_to != false) {
        $rows["table"]["add_link"] = route("notifications.create") . "?related_to=" . $related_to;
    }
    $rows["table"]["delete_link"] = url("dashboard/builder/notifications/");
    $rows["table"]["edit_link"] = url("dashboard/builder/notifications/");
...
function table_design($rows)
{
    ...
    $rows["action"]["delete_without"] = [1];
    $rows["action"]["view_without"] = [];
    $rows["action"]["links_without"] = [];
    $rows["action"]["name"] = "Notifications Controller";
    return $rows;
}
