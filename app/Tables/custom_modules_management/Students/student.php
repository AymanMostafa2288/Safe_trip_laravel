...
function table_head($rows)
{
    ...
    $rows['head_select'] = [
        'db'        => ['family_id'],
        'db_option' => ['family_id' => 'family_code'],
        'select'    => [],
    ];
    return $rows;
}
...
function table_body($rows, $related_to = false)
{
    ...
    $records_count = app(StudentInterface::class)->model->count();
    $page = 1;
    $offset = 0;
    $pagination = false;
    ...
    $body = app(StudentInterface::class)->data($request);
    $body = (array)json_decode(json_encode($body), true);
    $rows["body"] = $body;
    $rows["page"] = $page;
    $rows["pagination"] = $pagination;

    $rows["main_url"] = url("dashboard/modules/students/");
...
function table_buttons($rows, $related_to = false)
{
    ...
    $rows["table"]["delete_link"] = url("dashboard/modules/students/");
    $rows["table"]["edit_link"] = url("dashboard/modules/students/");
...
function table_design($rows)
{
    ...
    $rows["action"]["name"] = "Students Controller";
    return $rows;
}
