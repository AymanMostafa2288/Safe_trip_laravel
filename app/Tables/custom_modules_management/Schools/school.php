...
function table_body($rows, $related_to = false)
{
    ...
    $records_count = app(SchoolInterface::class)->model->count();
    $page = 1;
    $offset = 0;
    $pagination = false;
    ...
    $body = app(SchoolInterface::class)->data($request);
    ...
    $rows["main_url"] = url("dashboard/modules/schools/");
...
function table_buttons($rows, $related_to = false)
{
    ...
    $rows["table"]["add_link"] = route("schools.create");
    if ($related_to != false) {
        $rows["table"]["add_link"] = route("schools.create") . "?related_to=" . $related_to;
    }
    $rows["table"]["delete_link"] = url("dashboard/modules/schools/");
    $rows["table"]["edit_link"] = url("dashboard/modules/schools/");
...
function table_design($rows)
{
    ...
    $rows["action"]["name"] = "Schools Controller";
    return $rows;
}
