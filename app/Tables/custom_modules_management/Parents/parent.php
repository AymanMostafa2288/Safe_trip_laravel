...
function table_head($rows)
{
    $rows["head"] = [];
    $rows["head"]["id"] = "ID";
    $rows["head"]["name"] = "Family Name";
    $rows["head"]["code"] = "Family Code";
...
function table_body($rows, $related_to = false)
{
    ...
    $records_count = app(ParentsInterface::class)->model->count();
    $page = 1;
    $offset = 0;
    $pagination = false;
    ...
    $body = app(ParentsInterface::class)->data($request);
    ...
    $rows["main_url"] = url("dashboard/modules/parents/");
...
function table_buttons($rows, $related_to = false)
{
    ...
    $rows["table"]["add_link"] = route("parents.create");
    if ($related_to != false) {
        $rows["table"]["add_link"] = route("parents.create") . "?related_to=" . $related_to;
    }
    $rows["table"]["delete_link"] = url("dashboard/modules/parents/");
    $rows["table"]["edit_link"] = url("dashboard/modules/parents/");
...
function table_design($rows)
{
    ...
    $rows["action"]["name"] = "Parents Controller";
    return $rows;
}
