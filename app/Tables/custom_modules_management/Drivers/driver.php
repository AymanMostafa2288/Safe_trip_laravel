...
function table_body($rows, $related_to = false)
{
    ...
    $records_count = app(DriverInterface::class)->model->count();
    $page = 1;
    $offset = 0;
    $pagination = false;
    ...
    $body = app(DriverInterface::class)->data($request);
    ...
    $rows["main_url"] = url("dashboard/modules/drivers/");
...
function table_buttons($rows, $related_to = false)
{
    ...
    $rows["table"]["add_link"] = route("drivers.create");
    if ($related_to != false) {
        $rows["table"]["add_link"] = route("drivers.create") . "?related_to=" . $related_to;
    }
    $rows["table"]["delete_link"] = url("dashboard/modules/drivers/");
    $rows["table"]["edit_link"] = url("dashboard/modules/drivers/");
...
function table_design($rows)
{
    ...
    $rows["action"]["name"] = "Drivers Controller";
    return $rows;
}
