...
function table_head($rows)
{
    $rows["head"] = [];
    $rows["head"]["id"] = "ID";
    $rows["head"]["vehicle_number"] = "Vehicle Number";
...
function table_body($rows, $related_to = false)
{
    ...
    $records_count = app(BusInterface::class)->model->count();
    $page = 1;
    $offset = 0;
    $pagination = false;
    ...
    $body = app(BusInterface::class)->data($request);
    $body = (array)json_decode(json_encode($body), true);
    $rows["body"] = $body;
    $rows["page"] = $page;
    $rows["pagination"] = $pagination;
    $rows["main_url"] = url("dashboard/modules/buses/");
...
function table_buttons($rows, $related_to = false)
{
    ...
    $rows["table"]["add_link"] = route("buses.create");
    if ($related_to != false) {
        $rows["table"]["add_link"] = route("buses.create") . "?related_to=" . $related_to;
    }
    $rows["table"]["delete_link"] = url("dashboard/modules/buses/");
    $rows["table"]["edit_link"] = url("dashboard/modules/buses/");
...
function table_design($rows)
{
    ...
    $rows["action"]["name"] = "Buses Controller";
    return $rows;
}
...
function table_option(){
    $option                 = [];
    $option['status']       = BusStatusEnum::options();
    $option['status_color'] = [
        BusStatusEnum::WORK        => "gray",
        BusStatusEnum::MAINTENANCE  => "red",
    ];
    return $option;
}
