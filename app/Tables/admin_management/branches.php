...
function table_body($rows, $related_to = false)
{
    ...
    $body = app(BranchInterface::class)->data($request);
...
function table_buttons($rows, $related_to = false)
{
    ...
    $rows["table"]["add"] = true;
    $rows["table"]["add_link"] = route("branches.create");
    
    $rows["table"]["delete_link"] = url("dashboard/administrator/branches/");
    $rows["table"]["edit_link"] = url("dashboard/administrator/branches/");
...
function table_design($rows)
{
    ...
    $rows["action"]["name"] = "Branches Controller";
    return $rows;
}
