...
class CitiesController extends Controller
{
    ...
    {
        $table = getTable("locations_management/city");
        return viewBackend("global", "index", ["table" => $table, "config" => $this->config]);
    }

    public function create()
    {
        $form = getForm("locations_management/city");
        $this->config['breadcrumb'][]=[
            'title'=>'Create City',
            'route'=>'location.city.create',
            'query_builder'=>'',
        ];
    ...
    public function store(StoreCityRequest $request)
    {
        ...
        if($save_and_edit){
            $redirect=route("location.city.show",$return->id);
        }else{
            $redirect=route("location.city.index");
        }
