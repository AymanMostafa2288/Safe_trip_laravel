...
function form($data = []){
    ...
    $fields["left_2"] = [
        ...
        "password" => [
            ...
            "class" => "",
            "around_div" => "form-group form-md-line-input",
            "value" => (array_key_exists("password", $data)) ? $data["password"] : old("password")
        ],
        'active'=>[
            'input_type'=>'select',
            'title'=>'Is Active ?',
...
function form_attributes($fields, $id = ""){
    ...
    $fields["action"]         = ($id == "")? route("supervisors.store"):route("supervisors.update", $id);
    $fields["translate_href"] = url("dashboard/modules/supervisors/translate/" . $id);
    $fields["method"]         = "POST";
    $fields["class"]          = "";
    $fields["id"]             = $id;
