...
namespace App\Repositories\Eloquent\custom_modules_management\Parents;

use App\Models\CustomModels\Member;
...
use App\Models\CustomModels\Parents;
use Exception;
use File;
use DB;
...
class ParentRepository implements ParentsInterface
{
    public $model;
    private $files = [];
    private $images = [];
    ...
    public function save($request, $id = ""){
        ...
        try {
            ...
            foreach ($this->json as $json) {
                if (array_key_exists($json, $request)) {
                    ...
                    if (is_array($first_row)) {
                        ...
                        $array = ($request[$json][$key][0] != null) ?json_encode($request[$json]):json_encode([]);
                    }
                    $request[$json] = $array;
                } else {
                    $array          = json_encode([]);
            ...
            foreach ($this->elequant as $key => $value) {
                $inserted_values =[];
                foreach ($request[$key] as $name => $values){
                    foreach ($values as $index=>$db_value){
                        $inserted_values[$index][$name] = $values[$index];
                    }

                }
                foreach ($inserted_values as $inserted){
                    $inserted['family_id'] = $data->id;
                    $value::updateOrCreate(["family_id" => $id , 'national_id' => $inserted['national_id']],$inserted);
                }
    ...
    public function delete($id)
    {
        try {
            ...
            foreach ($this->elequant as $key => $value){
                $value::where('family_id',$id)->delete();
            }
            return true;
        } catch (Exception $e) {
            return false;
    ...
    public function translate($id)
    {
        ...
        $request = [];
        $request["where"][] = ["is_main" => 0];
        $fields = app(ModuleInterface::class)->data([], 1);
